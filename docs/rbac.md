...
### 5.3 契约演进：如何处理重构 (Evolution & Refactoring)
在长期开发中，前端组件的重命名是不可避免的。为了在不依赖后端迁移脚本的前提下保持系统稳定性，我们建议采用以下策略：

1.  **契约化思维 (Stability as Contract)**：
    将 `Component Code` 视为“技术契约”（如同数据库表名或 API 路径）。即便前端界面文案或文件路径发生变化，只要业务本质未变，代码标识应保持不动。
2.  **别名机制 (Aliasing)**：
    在前端注册表中引入 `aliases` 字段。重构时，新组件代码可以兼容旧代码。
    *   *逻辑*：如果用户的权限列表中包含 `old_code` 或 `new_code`，则该组件均视为授权。这样后端数据无需任何变动，即可实现平滑过渡。
3.  **自动发现与同步 (Auto-Discovery API)**：
    更高级的方案是实现一个 `POST /rbac/sync` 接口。前端在部署或管理员配置时，将最新的注册表全量同步给后端。通过映射逻辑，后端可以自动将角色中的旧标识迁移至新标识。

### 5.4 优雅下线：如何移除组件 (Safe Removal)
当一个功能不再需要时，移除组件的过程不仅需要保证权限的“干净”，还要兼顾并发操作的安全性：

1.  **前端下架 (Frontend Deprecation)**：
    开发者从 `registry.ts` 中删除组件定义。此时，该组件的 Code 在数据库中变为“无效数据”。
2.  **安全容错 (Data Tolerance)**：
    由于前端注册表已无该组件，UI 导游将不会渲染它。即便数据库中仍关联着该 Code，它也只是一个无意义的字符串，不会引发异常。如果该功能的 API 已改为对所有人开放或彻底移除，则不存在安全风险。
3.  **增量更新与隐式清理 (Incremental Cleanup & Concurrency)**：
    在更新角色权限时，为了防止多管理员并发操作导致的“覆盖丢失”，建议采用**增量 `PATCH`**（添加/移除特定 Code）而非全量 `PUT`。
    *   **触发机制**：当管理员下次打开权限配置页时，前端会对比“数据库返回的组件列表”与“当前代码注册表”。
    *   **自动修剪**：前端发现数据库中的 Code A 在注册表中已消失，会**自动标记**其为“待移除”。当用户点击保存时，前端会发送请求移除 Code A，从而完成隐式清理。

**核心逻辑**：通过前端注册表的“单一事实来源”，实现从 UI 屏蔽到数据库垃圾回收到权限池精简的完整闭环，且无需任何后端迁移。
...
### 6.3 运营风险与防御性设计 (Operational Risks)
隐式清理虽然优雅，但在复杂的分布式系统或多版本环境中存在以下潜在风险：

*   **多版本冲突 (Version Mismatch)**：假如管理员在使用 v2.0 (已删除组件 A)，而普通用户仍在使用缓存的 v1.5 (需要组件 A)。管理员点击保存后，组件 A 会在数据库层面全局注销，导致 v1.5 用户功能瞬间崩溃。
    *   *对策*：移除组件时，建议在注册表中保留其 Code 并标记为 `deprecated`，给予用户和缓存足够的过渡期，而非直接物理抹除。
*   **权限并集的所有权 (Union Recalculation Ownership)**：在使用增量 `PATCH` 操作时，谁负责重新计算最终的 API 权限池？
    *   *问题*：如果后端不感知 `registry.ts` 的映射关系，它将无法自动修剪因组件移除而失效的 API 权限。
    *   *对策*：后端应实现轻量级的权限重新校验逻辑，或者通过 BFF 层统一管理映射关系及其衍生的权限全量。
*   **幽灵权限期 (Ghost Permission Gap)**：组件从代码中移除后，到管理员点击保存前，数据库中仍残留旧的 API 权限。
    *   *风险*：技术人员仍可通过 Postman 等直接调用 API。
    *   *对策*：关键 API 的后端代码应进行逻辑加锁，而不应完全依赖权限拦截器。
*   **操作意图模糊 (Transparency of Intent)**：隐式清理会导致审计日志中出现非管理员主观操作的“权限移除”记录。
    *   *对策*：在 UI 界面显式提示：“系统发现已过时的组件（Code A），将在本次保存时自动清理”，将隐式行为转化为显式确认。
---
## 总结
| 阶段 | 核心机制 | 优点 | 缺点 |
| :--- | :--- | :--- | :--- |
